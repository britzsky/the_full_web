// /contact-api/ 경로 사용: nginx가 /api/contact/ 를 Spring Boot(8090)로 라우팅하므로
...
// 문의 답변 메일 미리보기 API 요청 본문 타입
type ContactReplyPreviewPayload = {
  content: string;
  userId?: string;
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  let body: Partial<ContactReplyPreviewPayload>;
  try {
    body = (await request.json()) as Partial<ContactReplyPreviewPayload>;
  } catch {
  ...
  if (!isCkEditorContentMeaningful(content)) {
    return NextResponse.json({ error: "미리보기할 답변 내용을 입력해 주세요." }, { status: 400 });
  }
  ...
  try {
    ...
    smtpConfigError = e instanceof Error ? e.message : String(e);
  }

  if (!smtpRuntimeConfig) {
